...
class PGD_Attack(object):
    ...
    def evaluate_attack(self, dl, model):
        ...
        ta_dl = myutils._to_dl(Xa, Ya, dl.batch_size)
        acc, loss = myutils.compute_loss_and_accuracy_from_dl(ta_dl, model,
                                                            F.cross_entropy,
                                                            device=self.device)
        return {
            'acc': acc.item(),
...
class L1_PGD_Attack(PGD_Attack):
    ...
    def get_norms(self, X):
        nch = len(X.shape)
        return X.view(X.shape[0], -1).abs().sum(dim=1)[(...,) + (None,)*(nch-1)]
    ...
    def _init_delta(self, xb, yb):
        # random vector with L1 norm rand_eps
