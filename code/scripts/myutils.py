...
def get_timestamp():
    ...
    return  ":".join(map(str, [d.year, d.month, d.day, d.hour, d.minute, d.second]))
...
def traverse_nested_dict(d, r=[], depth=np.inf):
    for k, v in d.items():
        if isinstance(v, collections.Mapping) and depth > 0:
            yield from traverse_nested_dict(v, r+[k], depth=depth-1)
        else:
            yield tuple(r+[k]), v
...
def split_dataloader(dl, frac=0.5):
    """returns dl0, dl1 with fractions p and 1-p"""
    bs = dl.batch_size
...
def _to_dl(X, Y, bs, shuffle=True, nw=0, pm=False, collate_fn=None):
    if collate_fn is None: collate_fn = default_collate
    return DataLoader(TensorDataset(torch.Tensor(X), torch.LongTensor(Y)), batch_size=bs, 
                      shuffle=shuffle, num_workers=nw, pin_memory=pm, collate_fn=collate_fn)
...
def _to_tensor_dl(dl, repeat=1, bs=None, nw=0, pm=False):
    X, Y = extract_numpy_from_loader(dl, repeat=repeat)
    dl = _to_dl(X, Y, bs if bs else dl.batch_size, nw=nw, pm=pm)
    return dl
...
def visualize_loader(dl, transform_fn=None):
    plt.figure(figsize=(20,4))
    for xb, yb in dl: break
    if transform_fn is not None:
        xb = transform_fn(xb)
    grid = torchvision.utils.make_grid(xb[:8])
...
def get_input_logit_gradients(X, Y, model, device=None, bs=250, flatten=False):
    ...
    if flatten: grads = grads.reshape(grads.shape[0], -1)
    return grads.numpy()

def get_input_loss_gradients(X, Y, model, device=None, bs=250, loss_fn=F.cross_entropy, loss_reduction='sum', flatten=False):
    if device is None: device = gu.get_device(None)
    model = model.to(device)
    grads = []
    ...
    for idx in sampler:
        ...
        out = model(xb)
        loss = loss_fn(out, yb, reduction=loss_reduction)
        loss.backward()
...
def pgd_adv_fit_model(model, opt, tr_dl, te_dl, attack, eval_attack=None, device=None, sch=None, max_epochs=100, epoch_gap=2, 
                      min_loss=0.001, print_info=True, save_init_model=True, loss_fn=F.cross_entropy):
    ...
    def l2_penalty(params, l2_lambda=0.001):
        """Returns the L2 penalty of the params."""
        l2_norm = sum((p ** 2).sum() for p in params)
        return l2_lambda * l2_norm

    def standard_epoch(loader, model, optimizer=None, sch=None, loss_fn=loss_fn):
        ...
        with torch.set_grad_enabled(update_params):
            for xb, yb in loader:
                ...
                loss = loss_fn(yp, yb)
                #loss = loss_fn(yp, yb) + l1_penalty(model.parameters())
                #loss = loss_fn(yp, yb) + l2_penalty(model.parameters())
    ...
    def adv_epoch(loader, model, attack, optimizer=None, sch=None, loss_fn=loss_fn):
...
# BEGIN L1 WEIGHT
def pgd_adv_fit_model_l1(model, opt, tr_dl, te_dl, attack, eval_attack=None, device=None, sch=None, max_epochs=100, epoch_gap=2, 
                      min_loss=0.001, print_info=True, save_init_model=True, loss_fn=F.cross_entropy):
...
# BEGIN L2 WEIGHT
def pgd_adv_fit_model_l2(model, opt, tr_dl, te_dl, attack, eval_attack=None, device=None, sch=None, max_epochs=100, epoch_gap=2, 
                      min_loss=0.001, print_info=True, save_init_model=True, loss_fn=F.cross_entropy):
...
# BEGIN Linf WEIGHT
def pgd_adv_fit_model_linf(model, opt, tr_dl, te_dl, attack, eval_attack=None, device=None, sch=None, max_epochs=100, epoch_gap=2, 
                      min_loss=0.001, print_info=True, save_init_model=True, loss_fn=F.cross_entropy):
    ...
    def linf_penalty(params, linf_lambda=0.001):
        """Returns the L-infinity penalty of the params."""
        linf_norm = max(p.abs().max() for p in params)
    ...
    def adv_epoch(loader, model, attack, optimizer=None, sch=None, loss_fn=loss_fn):
        ...
        for xb, yb in loader:
            ...
            with torch.set_grad_enabled(update_params):
                ...
                #loss = loss_fn(yp, yb) + l1_penalty(model.parameters())
                #loss = loss_fn(yp, yb) + l2_penalty(model.parameters())
                loss = loss_fn(yp, yb) + linf_penalty(model.parameters())
        ...
        try:
            ...
            for xb, yb in train_dl:
                ...
                if (num_evals == 0 or num_updates % act_update_gap == 0):
                    ...
                    val_acc = stats['acc_te'][-1] # best model is chosen based on accuracy
...
def update_ax(ax, title=None, xlabel=None, ylabel=None, legend_loc='best', ticks=True, ticks_fs=10, label_fs=12, 
              legend_fs=12, title_fs=14, hide_xlabels=False, hide_ylabels=False, despine=True, legend_cols=1):
    ...
    if legend_loc: ax.legend(loc=legend_loc, fontsize=legend_fs, ncol=legend_cols)
