...
class RandomSignalCoordinate(object):
    ...
    def get_dataloader(self, N, bs):
        ...
        self.dl = myutils._to_dl(self.X, self.Y, bs)
        return self.dl
    
    def get_input_gradients(self, model, device, bs=100, output='loss'):
        if output=='loss':
            G = myutils.get_input_loss_gradients(self.X, self.Y, model, device=device, 
                                               loss_reduction='sum', bs=100, flatten=False)
        elif output=='logit':
            G = myutils.get_input_logit_gradients(self.X, self.Y, model, device=device, bs=100)
        return G
        
    def get_model_predictions(self, model, device, bs=100):
        Yh = myutils.get_predictions_given_tensor(self.X, model, device=device, bs=bs).cpu()
        return Yh
